...
function GalleryScene({ images, speed = 1, visibleCount = 8, fadeSettings = { fadeIn: { start: 0.05, end: 0.15 }, fadeOut: { start: 0.85, end: 0.95 } }, blurSettings = { blurIn: { start: 0.0, end: 0.1 }, blurOut: { start: 0.9, end: 1.0 }, maxBlur: 3.0 } }) {
  ...
  const planesData = useRef(Array.from({ length: visibleCount }, (_, i) => ({ index: i, z: visibleCount > 0 ? ((depthRange / visibleCount) * i) % depthRange : 0, imageIndex: totalImages > 0 ? i % totalImages : 0, x: spatialPositions[i]?.x ?? 0, y: spatialPositions[i]?.y ?? 0 })))
  ...
  return (
    <>
      {planesData.current.map((plane, i) => {
        ...
        return (
          <ImagePlane key={plane.index} texture={texture} position={[plane.x, plane.y, worldZ]} scale={scale} material={material} />
        )
      })}
    </>
  )
...
function FallbackGallery({ images }) {
  ...
  return (
    <div style={{ padding: 16, background: '#f3f4f6' }}>
      <p style={{ color: '#374151', marginBottom: 12 }}>WebGL not supported. Showing image list:</p>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 12 }}>
        {normalizedImages.map((img, i) => (
          <img key={i} src={img.src || '/placeholder.svg'} alt={img.alt} style={{ width: '100%', height: 120, objectFit: 'cover', borderRadius: 8 }} />
        ))}
      </div>
    </div>
  )
...
export default function InfiniteGallery({ images, className = 'h-96 w-full', style, fadeSettings = { fadeIn: { start: 0.05, end: 0.25 }, fadeOut: { start: 0.4, end: 0.43 } }, blurSettings = { blurIn: { start: 0.0, end: 0.1 }, blurOut: { start: 0.4, end: 0.43 }, maxBlur: 8.0 } }) {
